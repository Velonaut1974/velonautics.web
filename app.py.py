...
# --- 2. CANONICAL PATH DEFINITION (Single Source of Truth) ---
...
# Fix für Fehler 2: Sicherstellen, dass die Klasse importiert wird
try:
    from portfolio import PortfolioCustodian
except ImportError:
    st.error("Datei portfolio.py nicht gefunden. Bitte prüfen!")

# --- 4. SERVICE INITIALIZATION ---
intake_service = IntakeService(LEDGER_DB_PATH)
...
def get_session_signer():
    """
    SECURITY CRITICAL: Retrieves the active private key for signing.
    Returns None if no key is loaded.
    """
    if 'private_key' not in st.session_state:
        return None
    
    signing_key = st.session_state['private_key']
    def signer(msg):
        return signing_key.sign(msg).signature
    return signer

def render_portfolio_module(ledger_instance):
    """
    Renders the Institutional Custody Interface (Module 10).
    Includes Architecture Integrity Checks.
    """
    st.header("Institutional Custody & Settlement")
    st.markdown("---")

    if not ledger_instance:
        st.error("SYSTEM HALT: Ledger not initialized.")
        return

    # 1. ARCHITECTURE INTEGRITY CHECK
    try:
        runtime_path = os.path.abspath(ledger_instance.db_path)
        config_path = os.path.abspath(GOVERNANCE_DB_PATH)
        if runtime_path != config_path:
            st.error(f"FATAL ARCHITECTURE ERROR: Runtime Path '{runtime_path}' != Config Path '{config_path}'")
            st.stop()
    except AttributeError:
        st.error("FATAL: Ledger Instance invalid.")
        st.stop()

    # 2. AUTH CHECK
    signer = get_session_signer()
    if not signer:
        st.error("AUTHENTICATION FAILURE: No active signing key found.")
        return

    # 3. INIT CORE & REPLAY
    # Hier wird der Import aus Block 1 jetzt genutzt -> Warnung verschwindet!
    custodian = PortfolioCustodian(GOVERNANCE_DB_PATH, ASSET_DB_PATH, ledger_instance)
    
    try:
        custodian.rebuild_state_from_ledger()
    except Exception as e:
        st.error(f"🛑 CRITICAL LEDGER STATE VIOLATION: {e}")
        st.stop()

    # 4. TABS
    tab_overview, tab_mint, tab_custody = st.tabs(["Portfolio Overview", "Asset Minting", "Custody Operations"])

    with tab_overview:
        rows = custodian.get_view_data()
        vol_open = sum([Decimal(r[1]) for r in rows if r[2] == "OPEN"])
        vol_res  = sum([Decimal(r[1]) for r in rows if r[2] == "RESERVED"])
        vol_tx   = sum([Decimal(r[1]) for r in rows if r[2] == "TRANSFERRED"])
        
        c1, c2, c3 = st.columns(3)
        c1.metric("Tradable Liquidity", f"{vol_open:,.2f} mt")
        c2.metric("Reserved / Escrow", f"{vol_res:,.2f} mt")
        c3.metric("Settled / Transferred", f"{vol_tx:,.2f} mt")
        
        if rows:
            st.dataframe([{"Asset Hash": r[0], "Volume": r[1], "Status": r[2]} for r in rows], width="stretch")
        else:
            st.info("Ledger contains no custody assets.")

    with tab_mint:
        candidates = custodian.get_minting_candidates()
        if not candidates:
            st.write("No new certified surplus blocks found.")
        else:
            st.write(f"Found {len(candidates)} un-minted certificates.")
            sel_hash = st.selectbox("Select Certificate", [c['hash'] for c in candidates], format_func=lambda x: f"...{x[-8:]}")
            if st.button("Mint Asset (PORTFOLIO_CREATE)"):
                try:
                    custodian.validate_and_write_block("PORTFOLIO_CREATE", {"asset_root_hash": sel_hash}, signer)
                    st.session_state['msg_succ'] = "Asset successfully minted."
                except Exception as e: st.error(f"Minting Rejected: {e}")

    with tab_custody:
        open_assets = [r for r in rows if r[2] == "OPEN"]
        res_assets = [r for r in rows if r[2] == "RESERVED"]
        action = st.radio("Operation Mode", ["Reserve", "Release"])
        
        if action == "Reserve":
            if open_assets:
                t = st.selectbox("Select OPEN Asset", [r[0] for r in open_assets])
                if st.button("Execute Reservation"):
                    try:
                        custodian.validate_and_write_block("PORTFOLIO_RESERVE", {"asset_root_hash": t, "reason": "MANUAL"}, signer)
                        st.session_state['msg_succ'] = "Reserved."
                    except Exception as e: st.error(f"Rejected: {e}")
            else: st.warning("No Assets.")

        elif action == "Release":
            if res_assets:
                t = st.selectbox("Select RESERVED Asset", [r[0] for r in res_assets])
                if st.button("Execute Release"):
                    try:
                        custodian.validate_and_write_block("PORTFOLIO_RELEASE", {"asset_root_hash": t, "reason": "MANUAL"}, signer)
                        st.session_state['msg_succ'] = "Released."
                    except Exception as e: st.error(f"Rejected: {e}")
            else: st.warning("No Assets.")

    if 'msg_succ' in st.session_state:
        st.success(st.session_state['msg_succ'])
...
def init_market_db():
    with sqlite3.connect(LEDGER_DB_PATH) as conn:
        cursor = conn.cursor()
        # 1. Marktdaten (Bleibt hier, da es Modul-unabhängig ist)
        cursor.execute('''
        ...
        # WICHTIG: Wir löschen hier die Erstellung von 'ledger_entries' und 'certified_receipts'.
        # Grund: Diese Tabellen werden jetzt exklusiv von ledger.py (Modul 10 Schema) 
        # und dem Asset-Commit-Block (Modul 4 Bridge) verwaltet.
        # Das verhindert den "Schema-Split-Brain".
...
# --- LEDGER INIT & SECURITY CHECK (UPDATED RC1) ---
if "ledger_bundle" not in st.session_state:
    try:
        ...
        # 2. Ledger Instanz erstellen (Mit RC1 Pfad)
        ...
        # 3. GENESIS BOOTSTRAP (Block 3 Integration)
        # Wir nutzen jetzt 'is_initialized' statt 'has_entries' (wegen neuer ledger.py)
        if not ledger_instance.is_initialized():
            # Lokaler Signer nur für diesen Moment
            def genesis_signer(msg): return signing_key.sign(msg).signature
            
            # Der neue RC1 Befehl
            ledger_instance.initialize_genesis(genesis_signer)
            
            # Feedback für dich beim ersten Start
            st.toast("⚓ SYSTEM GENESIS MINTED", icon="✅")

        # 4. Sicherheits-Check (Vergleich Key im Code vs. Key im Ledger)
        # (Dies verhindert, dass wir mit falschem Key auf eine bestehende DB zugreifen)
        # Hinweis: Da initialize_genesis nun 'is_initialized' nutzt, ist dieser Check implizit sicher,
        # aber verify_integrity macht den Rest.

        # 5. Integrität prüfen
        ledger_instance.verify_integrity()
        
        # Alles okay -> Bundle speichern
        st.session_state.ledger_bundle = (ledger_instance, signing_key, True, [])
        
    except Exception as e:
        # Fehler speichern -> UI zeigt das später an
...
certification_service = CertificationService(ledger, asset_engine)
...
try:
    if ledger is not None:
        ...
        is_valid = False
        chain_errors = chain_errors  # Bereits aus Bootstrap befüllt
except Exception as e:
    is_valid = False
    chain_errors = [str(e)]
...
# --- DEBUG: CERTIFICATION BRIDGE TEST (BLOCK B) ---
st.markdown("---")
st.write("### Debug: Certification Bridge Execution")

if st.button("EXECUTE TEST CERTIFICATION", type="primary"):
    # Dieser Aufruf nutzt den neuen Service, um einen Block in den Ledger zu schreiben
    cert_result = certification_service.create_certification("2026", lambda h: signing_key.sign(h).signature)
    
    if cert_result["status"] == "SUCCESS":
        st.success(cert_result["message"])
        st.code(f"Freeze Hash: {cert_result['freeze_hash']}", language="bash")
        
        # Prüfung, ob die Blockchain nach dem Schreiben noch integer ist
        if ledger.verify_integrity():
            st.info("Ledger Integrity Verified: TRUE")
        else:
            st.error("Ledger Integrity Check: FAILED")
    else:
        st.error(cert_result["message"])
        ...
        try:
            ...
            if results:
                    ...
                    if st.button("EXECUTE INSTITUTIONAL COMMIT", type="primary", width='stretch', key="btn_execute_gold"):
                        if not comment:
                            ...
                            if guard_result["status"] == "SUCCESS":
                                ...
                                pdf_bytes = generate_asset_pdf_from_block(block_record_sim)
                ...
                if 'msg_commit' in st.session_state:
                    ...
                    if st.button("✓ OK – Confirm Commit", key="ack_commit", type="primary"):
                        del st.session_state['msg_commit']
                        st.rerun()
...
# ==============================================================================
# MODULE 10: INSTITUTIONAL CUSTODY & SETTLEMENT
# ==============================================================================
# Status: ACTIVE | Architecture: Dual-Ledger Bridge
# Position: Downstream of Asset Generation, Upstream of Final Audit

# Wir nutzen die 'ledger' Instanz, die oben im Init-Block (Block 3) geladen wurde.
if 'ledger' in locals() and ledger is not None:
    render_portfolio_module(ledger)
else:
    # Fallback, falls der Init-Block übersprungen wurde (sollte nicht passieren)
    if 'ledger_bundle' in st.session_state:
        render_portfolio_module(st.session_state.ledger_bundle[0])
    else:
        st.warning("⚠️ Custody Module skipped: Ledger not operational.")
            ...
            if check:
                ...
                with col_audit1:
                    ...
                    if audit_trail:
                        ...
                        st.dataframe(df_audit, width="stretch", height=200)
                    else:
